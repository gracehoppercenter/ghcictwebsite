...
### Requirements
Using JavaScript and the Canvas API, create a simple game that meets the following requirements:
...
- **Interactivity:**
  - Implement at least one event handler (such as `keydown` and `keyup` events).
...
You are allowed to re-implement an existing game (such as Donkey Kong, Space Invaders, etc.)
, **but** all code and art that you submit must be your own original work.
